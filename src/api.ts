...
export const api = {
  ...
  getJurnal: async () => {
    ...
    return (data || []).map((j: any) => ({
      ...j,
      status_approval: j.status_approval || "Pending"
    }));
  },
  bulkApproveJurnal: async (ids: string[]) => {
    const { error } = await supabaseManual.from("jurnal").update({ status_approval: "Approved" }).in("id", ids);
  ...
  addSO: async (data: any) => {
    ...
    const KNOWN_COLS = new Set(["order_id", "no_invoice", "kode_invoice", "laporan_keuangan", "tgl_order",
      ...
      "is_posted", "bukti_muatan", "surat_jalan"]);
...
export const authActions = {
  ...
  updatePassword: async (oldPassword: string, newPassword: string) => {
    ...
    if (signInErr) throw new Error("Password lama tidak sesuai");
