...
import { C, I, STATUS_SO } from "../constants";
import { fmt, genJUNo, today } from "@/src/utils";
import { Card, SectionHeader, DBBanner, Spinner, EmptyState, useConfirm, PeriodFilter, Icon } from "@/src/components/SJMComponents";
...
export const JurnalUmum = ({ jurnal, setJurnal, coa, so, connected, currentUser, prefill, onPrefillUsed, onSOClick, onJurnalClick }: any) => {
  ...
  return (
    <div className="fade-up">
      ...
      <SectionHeader title={`Jurnal Umum (${jurnal.length})`} sub={`Menampilkan ${filtered.length} transaksi di periode ini`}
                        ...
                        <tr key={`${j.id}-${ei}`} className="row-hover" style={{ borderBottom: ei === details.length - 1 ? `1px solid ${C.border}` : "none" }}>
                          {ei === 0 && <>
                            ...
                            <td rowSpan={details.length} style={{ padding: "10px 14px" }}>
                              <button onClick={() => onJurnalClick && onJurnalClick(j.no_jurnal)} style={{ background: C.accentLight, color: C.accent, padding: "2px 7px", borderRadius: 5, fontSize: 11, fontWeight: 700, border: "none", cursor: "pointer" }}>{j.no_jurnal}</button>
                            </td>
                            <td rowSpan={details.length} style={{ padding: "10px 14px" }}>
      ...
      {tab === "input" && (
        <Card style={{ maxWidth: 800 }}>
            ...
            {form.entries.map((e: any, i: number) => (
              <div key={i} style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr 1.2fr 1.2fr 28px", gap: 10, marginBottom: 10 }}>
                ...
                <button onClick={() => {
                  ...
                  askConfirmJurnal({
                    title: "Hapus Baris",
                    msg: "Hapus baris akun ini?",
                    confirmLabel: "Hapus",
                    onConfirm: () => setForm((f: any) => ({ ...f, entries: f.entries.filter((_: any, idx: number) => idx !== i) }))
                  });
                }} style={{ color: C.red, background: "none", border: "none", cursor: form.entries.length <= 2 ? "not-allowed" : "pointer", fontSize: 18, opacity: form.entries.length <= 2 ? 0.3 : 1 }} title={form.entries.length <= 2 ? "Minimal 2 baris" : "Hapus baris"}>
                  ×
                </button>
              </div>
            ))}
            <button className="btn-ghost" onClick={() => setForm((f: any) => ({ ...f, entries: [...f.entries, { coa: "", akun: "", debit: "", kredit: "", no_so: "" }] }))} style={{ width: "100%" }}><Icon d={I.plus} size={12} style={{ marginRight: 8 }} /> Tambah Baris Transaksi</button>
          </div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: 16, background: balanced ? C.greenLight : C.bg, borderRadius: 10, marginBottom: 20 }}>
            ...
            {balanced ? <span style={{ color: C.green, fontWeight: 700, display: "flex", alignItems: "center", gap: 6 }}><Icon d={I.check} size={16} /> Seimbang</span> : <span style={{ color: C.red, fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}><Icon d={I.alert} size={16} /> Belum Seimbang</span>}
          </div>
