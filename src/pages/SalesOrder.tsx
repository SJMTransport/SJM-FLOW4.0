...
const parseRupiah = (v: string): number => {
  ...
  const s = v.trim();
  if (!s || s === "-" || s.toLowerCase() === "#ref!") return 0;
  // Strip currency prefix, spaces, thousand separator (dots), convert decimal comma
  const clean = s.replace(/Rp/gi, "").replace(/\s/g, "").replace(/\./g, "").replace(",", ".");
...
const parseCSV = (text: string): { headers: string[]; rows: any[] } => {
  ...
  // Reconstruct lines handling quoted newlines
  const lines: string[] = [];
  let cur = "", inQ = false;
  ...
  // Parse headers — strip BOM, newlines, quotes from each header
  const rawHeaders = parseRow(lines[0], sep);
  const headers = rawHeaders.map(h => h.replace(/"/g, "").replace(/\n/g, " ").trim());
  ...
  const rows = lines.slice(1).map(line => {
    ...
    // Accept rows where Order ID starts with SJM (check multiple possible header names)
    const id = r["Order ID (Auto)"] || r["order_id"] || r["Order ID"] || r["No SO"] || "";
    return id.startsWith("SJM");
  });
...
const mapCSVRow = (row: any): any => {
  // Flexible getter — checks multiple possible header names
  const g = (...keys: string[]): string => {
    for (const k of keys) {
      const v = row[k];
      if (v !== undefined && v !== "" && v !== "#REF!") return v;
    }
    return "";
  };

  const order_id = g("Order ID (Auto)", "order_id", "Order ID", "No SO");

  return {
    order_id,
    no_invoice: g("No Invoice", "no_invoice") || null,
    kode_invoice: g("Kode Invoice", "kode_invoice") || null,
    customer: g("Customer"),
    pic: g("PIC Cust", "pic", "PIC") || null,
    no_pic: g("No PIC", "no_pic") || null,
    tgl_order: parseIDDate(g("Tanggal Order", "tgl_order", "Tgl Order")),
    tgl_muat: parseIDDate(g("Tanggal Muat", "tgl_muat", "Tgl Muat")),
    jam_muat: g("Jam Muat ", "Jam Muat", "jam_muat") || null,
    lokasi_muat: g("Lokasi Muat", "lokasi_muat"),
    sharelok_muat: g("Sharelok Muat", "sharelok_muat") || null,
    lokasi_bongkar: g("Lokasi Bongkar", "lokasi_bongkar"),
    sharelok_bongkar: g("Sharelok Bongkar", "sharelok_bongkar") || null,
    nama_sopir: g("Nama Sopir", "nama_sopir", "Sopir") || null,
    jenis_truk: g("Jenis Truk", "jenis_truk") || null,
    no_polisi: g("No polisi", "No Polisi", "no_polisi") || null,
    no_supir: g("No Supir", "no_supir") || null,
    armada: g("Armada", "armada") || null,
    unit_muatan: g("Unit Muatan", "unit_muatan") || null,
    base_harga: parseRupiah(g("Base Harga", "base_harga")),
    harga_asuransi: g("Harga Asuransi", "harga_asuransi") || null,
    pajak: g("Pajak", "pajak") || null,
    nilai_pajak: parseRupiah(g("Nilai Pajak", "nilai_pajak")),
    harga_pengiriman: parseRupiah(g("Harga Pengiriman", "harga_pengiriman")),
    total_harga: parseRupiah(g("Total Harga", "total_harga")),
    total_harga_pajak: parseRupiah(g("Total Harga + Pajak", "total_harga_pajak", "Total + Pajak")),
    status_muatan: g("Status Muatan", "status_muatan") || "Order Confirmed",
    tgl_bongkar: parseIDDate(g("Tanggal Bongkar", "tgl_bongkar", "Tgl Bongkar")) || null,
    no_asuransi: g("No Asuransi", "no_asuransi") || null,
    nilai_tanggungan: parseRupiah(g("Nilai Tanggungan Asuransi", "nilai_tanggungan")),
    nilai_asuransi: parseRupiah(g("Nilai Asuransi", "nilai_asuransi")),
    nilai_tanpa_asuransi: parseRupiah(g("Nilai Tanpa Asuransi", "nilai_tanpa_asuransi")),
