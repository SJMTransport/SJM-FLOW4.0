    ...
    <Card className="w-full max-w-5xl h-full flex flex-col p-0 border-none rounded-none bg-[#F5F4F1] overflow-hidden shadow-2xl animate-fade-left relative z-10" onClick={e => e.stopPropagation()}>
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar bg-slate-50/30">
        <div className="grid grid-cols-12 min-h-full divide-y lg:divide-y-0 lg:divide-x divide-border-main/30">
          ...
          <div className="col-span-12 lg:col-span-4 p-6 space-y-5">
            <div className="flex items-center gap-2 pb-2 border-b border-border-main/30">
              <div className="w-1.5 h-3 bg-accent rounded-full" />
              <h4 className="text-[9px] font-black text-text-light uppercase tracking-widest">Logistik & Armada</h4>
            </div>

            {/* Kartu Customer */}
            <div className="bg-white rounded-xl border border-border-main/60 p-4 space-y-3 shadow-sm">
              <div>
                <div className="text-[8px] font-bold text-text-light uppercase tracking-widest mb-0.5">Customer</div>
                <div className="text-[13px] font-black text-text-main leading-tight">{data.customer}</div>
              </div>
                ...
                <div className="pt-2 border-t border-border-main/20">
                  ...
                  {data.no_pic && (
                    <div className="mt-1 flex items-center justify-between gap-2">
                      <span className="text-[10px] text-text-med font-mono">{data.no_pic}</span>
                      <a
            ...
            <div className="bg-white rounded-xl border border-border-main/60 p-4 space-y-3 shadow-sm">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <button
                    ...
                    className="text-[12px] font-black text-accent hover:underline uppercase tracking-tight block text-left font-mono"
                  >
                    {data.no_polisi || "—"}
                  </button>
              ...
              <div className="pt-2 border-t border-border-main/20">
                <div className="text-[8px] font-bold text-text-light uppercase tracking-widest mb-0.5">Expedisi / Vendor</div>
                <div className="text-[11px] font-bold text-text-main">{data.nama_vendor || "—"}</div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-border-main/60 p-4 space-y-3 shadow-sm">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <div className="text-[12px] font-bold text-text-main leading-snug">{data.muatan || "—"}</div>
                </div>
                <div>
                  <div className="text-[8px] font-bold text-text-light uppercase tracking-widest mb-0.5">Kuantitas / Tonase</div>
                  <div className="text-[12px] font-bold text-text-main leading-snug">{data.unit_muatan || (data.tonase ? `${data.tonase} ton` : "—")}</div>
                </div>
              </div>
              {data.sn && (
                <div className="pt-2 border-t border-border-main/20">
            ...
            {/* Keterangan SO */}
            {data.keterangan && (
              <div className="bg-amber-50/50 p-3.5 rounded-xl border border-amber-200/50">
                <div className="text-[8px] font-bold text-amber-800 uppercase tracking-widest mb-1.5">Catatan Khusus</div>
                <div className="text-[10px] font-medium text-amber-900 italic leading-relaxed">{data.keterangan}</div>
              </div>
            )}
          </div>
          ...
          <div className="col-span-12 lg:col-span-4 p-6 space-y-5">
            <div className="flex items-center gap-2 pb-2 border-b border-border-main/30">
              ...
              <h4 className="text-[9px] font-black text-text-light uppercase tracking-widest">Rute & File Perjalanan</h4>
            </div>
            ...
            <div className="bg-white rounded-xl border border-border-main/60 p-4 space-y-3 shadow-sm">
              ...
              <div className="flex items-start gap-2.5">
                <div className="flex-1 min-w-0">
                  <div className="text-[11px] font-black text-text-main leading-tight truncate" title={data.lokasi_muat}>{data.lokasi_muat || "—"}</div>
                  {data.sharelok_muat && (
                    <a href={data.sharelok_muat} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[8px] text-accent hover:underline font-black mt-1 uppercase tracking-wider">
                ...
                <div className="flex-1 min-w-0 text-right">
                  <div className="text-[11px] font-black text-text-main leading-tight truncate" title={data.lokasi_bongkar}>{data.lokasi_bongkar || "—"}</div>
                  {data.sharelok_bongkar && (
                    <a href={data.sharelok_bongkar} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[8px] text-accent hover:underline font-black mt-1 uppercase tracking-wider">
                      Google Maps <Icon name="MapPin" size={8} />
                    </a>
                  )}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3 pt-2 border-t border-border-main/20">
                <div>
                  <div className="text-[8px] font-bold text-text-light uppercase tracking-widest mb-0.5">Tanggal Muat</div>
                  <div className="text-[11px] font-bold text-text-main tabular-nums">{data.tgl_muat || "—"} {data.jam_muat && `@ ${data.jam_muat}`}</div>
                </div>
                <div>
                  <div className="text-[8px] font-bold text-text-light uppercase tracking-widest mb-0.5">Tanggal Bongkar</div>
                  <div className="text-[11px] font-bold text-text-main tabular-nums">
                    {data.tgl_bongkar || "—"}
            ...
            {(() => {
              ...
              return (
                <div className="bg-white rounded-xl border border-border-main/60 p-4 space-y-2.5 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {allDocs.map((doc, idx) => {
                      ...
                      return (
                        <div key={idx} className={`p-2 rounded-lg border ${colorHex} flex items-center justify-between gap-2`}>
            ...
            <div className="bg-white rounded-xl border border-border-main/60 p-4 space-y-3.5 shadow-sm">
              <div className="flex justify-between items-center text-[8px] font-bold text-text-light uppercase tracking-widest opacity-60">
              ...
              {(!data.posisi_log || data.posisi_log.length === 0) ? (
                ...
                <div className="border-l border-border-main/40 ml-1.5 space-y-3 text-left">
                  {(data.posisi_log || []).map((log: any, idx: number) => {
                    ...
                    return (
                      <div key={idx} className="relative pl-4 last:pb-0">
                        ...
                        <div className="text-[10px] font-black text-text-main flex items-center gap-1.5 leading-none">
                          <span>{log.location || log.status}</span>
                          {log.status && <span className="text-[7px] font-black px-1.5 py-0.5 rounded uppercase" style={{ backgroundColor: dotColor + "15", color: dotColor }}>{log.status}</span>}
                        </div>
                        {log.info && !log.info.includes("Status diperbarui via Stepper") && <div className="text-[9px] text-text-med leading-snug mt-1">{log.info}</div>}
          ...
          <div className="col-span-12 lg:col-span-4 p-6 space-y-5">
            <div className="flex items-center gap-2 pb-2 border-b border-border-main/30">
              ...
              <h4 className="text-[9px] font-black text-text-light uppercase tracking-widest">Keuangan & Invoice</h4>
            </div>

            {/* Rincian Biaya */}
            <div className="bg-white rounded-xl border border-border-main/60 p-4 space-y-2.5 shadow-sm">
              <div className="text-[8px] font-bold text-text-light uppercase tracking-widest opacity-60">Rincian Biaya & Pajak</div>
              <div className="space-y-1.5 text-[11px] font-bold text-text-med">
                ...
                <div className="flex justify-between items-center pt-2 mt-1 border-t border-border-main/30">
                  ...
                  <span className="text-[14px] font-black text-text-main tabular-nums">{fmt(nilaiSO)}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-border-main/60 p-4 space-y-3.5 shadow-sm">
              ...
              {totalPiutang > 0 && (
                <div>
                  <div className="flex justify-between text-[9px] text-text-light mb-1.5 font-bold">
                ...
                {totalPiutang > 0 ? (
                  <>
                    ...
                    <div className="flex justify-between items-center pt-2 border-t border-border-main/20">
                      ...
                      <span className="text-[13px] font-black tabular-nums" style={{ color: payColor }}>{fmt(sisaPiutang)}</span>
                    </div>
                  </>
                ) : (
            ...
            {(data.no_asuransi || data.nilai_tanggungan || data.spk) && (
              <div className="bg-white rounded-xl border border-border-main/60 p-4 space-y-2.5 shadow-sm">
                ...
                <div className="space-y-1.5 text-[11px] font-bold text-text-med">
                  ...
                  {data.spk && (
                    <div className="flex justify-between items-center pt-1.5 border-t border-border-main/20">
            ...
            <div className="bg-white rounded-xl border border-border-main/60 p-4 space-y-3 shadow-sm">
              <div className="flex justify-between items-center text-[8px] font-bold text-text-light uppercase tracking-widest opacity-60">
                ...
                <span>{relatedInvoices.length} Invoice</span>
              </div>
              {relatedInvoices.length === 0 ? (
                <div className="rounded-xl bg-amber-50/50 border border-amber-200/50 p-3 text-center">
                  ...
                  <div className="text-[8px] text-amber-600 opacity-80 mt-0.5 mb-2">SO ini belum diterbitkan tagihannya</div>
                      ...
                      <div key={inv.id} className="p-2.5 rounded-lg border border-border-main/60 flex items-center justify-between gap-3 hover:bg-slate-50 transition-all">
                        <div className="min-w-0">
                          <button
                            ...
                            className="text-[10px] font-black text-accent uppercase tracking-tight hover:underline text-left block truncate max-w-[150px]"
                          >
                            {inv.no_invoice}
                          </button>
                          <div className="text-[8px] text-text-light opacity-50 tabular-nums mt-0.5">{fmtDate(inv.tgl_invoice)}</div>
                        </div>
                        <div className="text-right shrink-0">
                          <div className="text-[10.5px] font-black text-text-main tabular-nums">{fmt(inv.total_setelah_pajak || 0)}</div>
                          <span className="text-[7.5px] font-black px-1.5 py-0.5 rounded-full uppercase block w-fit ml-auto mt-0.5" style={{ backgroundColor: sc + '15', color: sc }}>{inv.status_bayar || 'Belum Bayar'}</span>
                        </div>
                      </div>
                    );
                  })}
                  ...
                  {relatedJurnals.map((j: any) => (
                    <div key={j.id} onClick={() => onJurnalClick && onJurnalClick(j.no_jurnal)} className="p-2.5 rounded-lg border border-border-main/60 hover:bg-slate-50 transition-all cursor-pointer flex items-center justify-between gap-2 group">
                      <div className="min-w-0 flex-1">
                        <span className="text-[10px] font-black text-accent uppercase tracking-wide group-hover:underline block">{j.no_jurnal}</span>
                        <span className="text-[9px] text-text-med truncate block w-full mt-0.5">{j.keterangan}</span>
                      </div>
                      <div className="text-right shrink-0">
                        <span className="text-[11px] font-black text-text-main tabular-nums leading-none block">{fmt(j.total_debit)}</span>
            ...
            {/* Aksi Cepat */}
            <div className="bg-white rounded-xl border border-border-main/60 p-4 space-y-2 shadow-sm">
              <div className="text-[8px] font-bold text-text-light uppercase tracking-widest opacity-60 mb-2">Aksi Cepat</div>
              {canEdit && (
                <button
                  onClick={() => { handleNav("operasional", "so"); setPendingEditSO(data.order_id); onClose(); }}
                  className="w-full h-8 rounded-lg border border-border-main text-[9px] font-black text-text-med uppercase tracking-widest hover:border-accent hover:text-accent hover:bg-accent/5 transition-all flex items-center justify-center gap-1.5"
                >
              ...
              {data.status_muatan === 'Completed' && relatedInvoices.length === 0 && (
                <button
                  onClick={() => { handleNav("operasional", "invoice"); onClose(); }}
                  className="w-full h-8 rounded-lg bg-accent text-white text-[9px] font-black uppercase tracking-widest hover:bg-accent/90 transition-all flex items-center justify-center gap-1.5"
                >
                  <Icon name="FileText" size={10} /> Buat Invoice
                </button>
              )}
              {(canEdit || canFinance) && (
                <button
              ...
              <button
                onClick={onClose}
                className="w-full h-8 rounded-lg border border-border-main/40 text-[9px] font-medium text-text-light uppercase tracking-widest hover:bg-slate-50 transition-all"
              >
                Tutup Panel
              </button>
            </div>
